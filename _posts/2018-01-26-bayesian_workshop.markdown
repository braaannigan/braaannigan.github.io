...
To make sure that we are all using the same software we will download Anancona.
Anaconda is a distribution of python -- it has all the files needed to run
python on your machine -- and it also provides an *environment manager* called
...
is running because to the right of every cell you see
```
In [*]
```
...
In []
```
with nothing in the square brackets, it means that an error has occurred.  In this
case scroll back up to the top to find the error message.  Some possible errors
and corrections to them are set out below.
...
```
Alternatively, it may be ```pymc3``` or ```holoviews``` that is not found.
...
```
This error can be addressed by adding the following line to the first cell
block of the notebook (with the second line added in to show where it goes)
```
